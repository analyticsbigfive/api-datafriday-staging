...
# Test simple pour récupérer les événements avec organization ID 182509
...
if [ "$HAS_DATA" = "true" ]; then
  ...
  if [ "$EVENT_COUNT" -gt 0 ]; then
    ...
    echo "💡 Utilisez le script test-events-182509-fixed.sh pour synchroniser les événements"
  fi
else
  echo "📄 Réponse brute:"
  echo "$EVENTS" | jq .
